...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ClaimTypesConfig.ascx.cs" Inherits="Yvand.EntraClaimsProvider.Administration.ClaimTypesConfigUserControl" %>
<%@ Import Namespace="Yvand.EntraClaimsProvider.Administration" %>
...
<style type="text/css">
    ...
    .entracp-rowidentityclaim {
        font-weight: bold;
        color: green;
    }

    .entracp-rowClaimTypeNotUsedInTrust {
        color: red;
        font-style: italic;
		text-decoration: line-through;
    }

    .entracp-rowUserProperty {
        color: green;
    }
	
	.entracp-rowMainGroupClaimType {
		font-weight: bold;
        color: #0072c6;
    }

    .entracp-rowGroupProperty {
        color: #0072c6;
    }
...
<script type="text/javascript">
    ...
    window.Entracp.ClaimsTablePage = window.Entracp.ClaimsTablePage || {};

    // Hide labels and show input controls
    window.Entracp.ClaimsTablePage.EditItem = function (ItemId) {
    ...
    window.Entracp.ClaimsTablePage.CancelEditItem = function (ItemId) {
    ...
    _spBodyOnLoadFunctionNames.push("window.Entracp.ClaimsTablePage.Init");

    window.Entracp.ClaimsTablePage.Init = function () {
        // Variables initialized from server side code
        window.Entracp.ClaimsTablePage.ShowNewItemForm = <%= ShowNewItemForm.ToString().ToLower() %>;
        window.Entracp.ClaimsTablePage.HideAllContent = <%= HideAllContent.ToString().ToLower() %>;
        window.Entracp.ClaimsTablePage.TrustName = "<%= TrustName.ToString().ToLower() %>";

        // Check if all content should be hidden (most probably because EntraCP is not associated with any SPTrustedLoginProvider)
        if (window.Entracp.ClaimsTablePage.HideAllContent) {
            $('#divMainContent').hide();
            return;
        }
        ...
        $('#divMainContent').find("span").each(function (ev) {
            $(this).text($(this).text().replace("{trustname}", window.Entracp.ClaimsTablePage.TrustName));
        });

        // ONLY FOR SP 2013: Force display of current page in full screen mode
        window.Entracp.ClaimsTablePage.SetFullScreenModeInCurPageOnly();

        // Initialize display
        var rdbGroupName = $('#<%= RdbNewItemClassicClaimType.ClientID %>').attr('name');
        if (Entracp.ClaimsTablePage.ShowNewItemForm) {
    ...
    window.Entracp.ClaimsTablePage.SetFullScreenModeInCurPageOnly = function () {
...
<div id="divMainContent">
    ...
    <div id="divBtnsFullScreenMode">
        ...
        <input id="btnEnableFullScreenMode" type="button" value="Maximize content" onclick="window.Entracp.ClaimsTablePage.SetFullScreenModeInCurPageOnly(); $('#btnEnableFullScreenMode').hide(); $('#btnDisableFullScreenMode').show(); return false;" style="display: none;" class="ms-ButtonHeightWidth" />
        <input type="button" value="Refresh page" onClick="window.location.href = window.location.href; return false;">
    </div>
    <div id="divTblClaims">
        <span style="display: block; margin-bottom: 10px;">This table shows the mappings between the claim types (initially defined in SPTrustedIdentityTokenIssuer &quot;{trustname}&quot;) and the Microsoft Entra ID objects and properties.</span>
        <asp:Table ID="TblClaimsMapping" runat="server"></asp:Table>
        <div id="divLegend">
            <fieldset>
                <legend>Formatting legend:</legend>
                <ol>
                    <li><span class="entracp-rowidentityclaim">This formatting</span><span> shows the main &quot;User&quot; mapping, between Microsoft Entra ID object &quot;User&quot; and the identity claim type set in SPTrust &quot;{trustname}&quot;. It is required for EntraCP to work.</span></li>
                    <li><span class="entracp-rowUserProperty">This formatting</span><span> shows an Microsoft Entra ID user property used only in the query to Microsoft Entra ID. Permission is created using the main &quot;User&quot; mapping.</span></li>
                    <li><span class="entracp-rowMainGroupClaimType">This formatting</span><span> shows the main &quot;Group&quot; mapping, between Microsoft Entra ID object &quot;Group&quot; and a claim type. EntraCP supports only 1 mapping for &quot;Group&quot; type.</span></li>
					<li><span class="entracp-rowGroupProperty">This formatting</span><span> shows an Microsoft Entra ID group property used only in the query to Microsoft Entra ID. Permission is created using the main &quot;Group&quot; mapping.</span></li>
					<li><span class="entracp-rowClaimTypeNotUsedInTrust">This formatting</span><span> shows a claim type not set in SPTrust &quot;{trustname}&quot;, it is ignored by EntraCP and can be safely deleted.</span></li>
                </ol>
            </fieldset>
        </div>
        ...
        <fieldset>
            ...
            <ol>
                ...
                <div id="divNewItemControls" style="display: none;">
					...
					<li>
                        <label for="<%= DdlNewDirectoryObjectType.ClientID %>">Microsoft Entra ID object type: <em>*</em></label>
                        <asp:DropDownList ID="DdlNewDirectoryObjectType" runat="server" CssClass="ms-inputformcontrols"></asp:DropDownList>
                    </li>
                    <li>
                        <label for="<%= DdlNewGraphProperty.ClientID %>">Microsoft Entra ID object property to query: <em>*</em></label>
                    ...
                    <li id="rowGraphPropertyToDisplay" style="display: none;">
                        <label for="<%= DdlNewGraphPropertyToDisplay.ClientID %>">Microsoft Entra ID object property to display:</label>
